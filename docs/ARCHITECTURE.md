...
Windows использует `WH_KEYBOARD_LL`, `ToUnicodeEx`, `SendInput` и `WM_INPUTLANGCHANGEREQUEST`. macOS использует CGEvent tap/post и TIS input source. Обе реализации поддерживают настраиваемую диктовку и отмену.
